...
def fset_reduced(pr, fset, sr, cypoly, polys, invol, ni, rwn=None):
    n = pr.ngens()
    x = pr.gens()
    ...
    for i in range(k):
        ...
        if cypoly.reduce(idp) == 0:
            ...
            fset2.pop(i)
            return fset_reduced(pr, fset2, sr, cypoly, polys, invol, ni, rwn)
    ...
    cyset = fsetx + [cypoly]
    for sec in secs:
        gset = cyset + sec + [x[involindset[0]]-x[involindset[1]] for involindset in invol]
        ig = pr.ideal(gset)
        # print(ig)
    ...
    for i in range(k-1,-1,-1):
        ...
        for comb in lst:
            ...
            for j in range(len(secs)):
                sec = secs[j]
                gset = genset + sec + [x[involindset[0]]-x[involindset[1]] for involindset in invol]
                #print(gset)
                igen = pr.ideal(gset)
                #print(igen)
                # print(igen.groebner_basis())
                d = igen.dimension()#len(igen.groebner_basis())
                print([i,j,dimens[j],d])
                sys.stdout.flush()
                if (dimens[j] > -1) and (d != dimens[j]):
                    test = False
                    break
            if not test:
                return fsetx, prevcomb
            else:
                prevcomb = comb

    return fsetx, currcomb
...
def main_one(polyid, geonum, trinum, invnum, h11, h21, invol, basisinds, dresverts, sr, rwmat):
    ...
    if q > k:
        ...
        for i in range(len(fsets)):
            fset = fsets[i]
            fsetx = fsetsx[i]
            fset, fr = fset_reduced(prx, fset, sr, symcypoly, polys, invol, ni, rwn)
        ...
        # Sort the fixed sets into O7, O5, O3, O1 by finding the codimension
        for fset in fsets:
            fset, fr = fset_reduced(pr, fset, sr, symcypoly, polys, invol, ni, rwn)
