...
def main_one(polyid, geonum, trinum, invnum, h11, h21, invol, basisinds, dresverts, sr, rwmat):
    ...
    if q > k:
        ...
        frs = []
        fsetsx2 = []
        for i in range(len(fsetsx)):
            #fset = fsets[i]
            #print(fset)
            fsetx = fsetsx[i]
            fsetx, fr = fset_reduced(prx, fsetx, sr, symcypoly, polys, ni, rwn)
            fsetsx2.append(fsetx)
            frs.append(fr)
        ...
        for p1 in range(len(frs)):
            ...
            for p2 in range(len(frs)):
                ...
                if sublist_of(frs[p1], frs[p2]) and p1 != p2:
                    remove.append(p2)
        fsetsx2 = [fsetsx2[w] for w in range(len(fsetsx2)) if w not in remove]
